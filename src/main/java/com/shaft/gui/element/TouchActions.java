...
public class TouchActions {
	...
	/**
	 * This is a convenience method to be able to call Element Actions from within
	 * the current Touch Actions instance.
	 * <p>
	 * Sample use would look like this: new
	 * TouchActions(driver).tap(username_textbox).performElementAction().type(username_textbox,
	 * "username");
	 *
	 ...
	 *
	 * @param elementReferenceScreenshot relative path to the reference image from
	 *                                   the local object repository, ends with /
	 * @return a self-reference to be used to chain actions
	 */
	public TouchActions tap(String elementReferenceScreenshot) {
		...
		if (DriverFactoryHelper.isMobileNativeExecution()) {
			...
			List<Integer> coordinates = ImageProcessingActions.findImageWithinCurrentPage(elementReferenceScreenshot,
					currentScreenImage, 1);
			PointerInput input = new PointerInput(PointerInput.Kind.TOUCH, "finger1");
			Sequence tap = new Sequence(input, 0);
			tap.addAction(input.createPointerMove(Duration.ZERO, PointerInput.Origin.viewport(), coordinates.get(0),
					coordinates.get(1)));
			...
			(new org.openqa.selenium.interactions.touch.TouchActions(driver))
					.down(coordinates.get(0), coordinates.get(1)).up(coordinates.get(0), coordinates.get(1)).perform();
		}
	...
	public TouchActions tap(By elementLocator) {
		...
		if (WebDriverElementActions.identifyUniqueElement(driver, internalElementLocator)) {
			...
			List<Object> screenshot = WebDriverElementActions.takeScreenshot(driver, internalElementLocator, "tap",
					null, true);
			...
			try {
				if (driver instanceof AppiumDriver<?>) {
					...
					(new org.openqa.selenium.interactions.touch.TouchActions(driver))
							.singleTap(driver.findElement(internalElementLocator)).perform();
				}
			} catch (Exception e) {
	...
	public TouchActions doubleTap(By elementLocator) {
		...
		if (WebDriverElementActions.identifyUniqueElement(driver, internalElementLocator)) {
			...
			List<Object> screenshot = WebDriverElementActions.takeScreenshot(driver, internalElementLocator,
					"doubleTap", null, true);
			// takes screenshot before clicking the element out of view
			...
			try {
				if (driver instanceof AppiumDriver<?>) {
					...
					// regular touch screen device
					(new org.openqa.selenium.interactions.touch.TouchActions(driver))
							.doubleTap(driver.findElement(internalElementLocator)).perform();
				}
			} catch (Exception e) {
				WebDriverElementActions.failAction(driver, internalElementLocator, e);
			}
		...
		if (WebDriverElementActions.identifyUniqueElement(driver, internalElementLocator)) {
			...
			try {
				if (driver instanceof AppiumDriver<?>) {
					...
					// regular touch screen device
					(new org.openqa.selenium.interactions.touch.TouchActions(driver))
							.longPress(driver.findElement(internalElementLocator)).perform();
				}
			} catch (Exception e) {
				WebDriverElementActions.failAction(driver, internalElementLocator, e);
			}
	...
	/**
	 * Send the currently active app to the background, and return after a certain
	 * number of seconds.
	 * 
	 * @param secondsToSpendInTheBackground number of seconds before returning back
	 *                                      to the app
	 * @return a self-reference to be used to chain actions
	 */
	...
	/**
	 * Send the currently active app to the background and leave the app
	 * deactivated.
	 * 
	 * @return a self-reference to be used to chain actions
	 */
	...
	/**
	 * Activates an app that has been previously deactivated or sent to the
	 * background.
	 * 
	 * @param appPackageName the full name for the app package that you want to
	 *                       activate. for example [com.apple.Preferences] or
	 ...
	 * quits the session. Then re-Launches the app and restarts the session.
	 * 
	 * @return a self-reference to be used to chain actions
	 */
	...
	public TouchActions swipeToElement(By sourceElementLocator, By destinationElementLocator) {
		...
		if (WebDriverElementActions.identifyUniqueElement(driver, internalSourceElementLocator)
			...
			// Override current locator with the aiGeneratedElementLocator
			internalSourceElementLocator = WebDriverElementActions
					.updateLocatorWithAIGeneratedOne(internalSourceElementLocator);
			internalDestinationElementLocator = WebDriverElementActions
					.updateLocatorWithAIGeneratedOne(internalDestinationElementLocator);
			...
			try {
				if (driver instanceof AppiumDriver<?>) {
					...
					// regular touch screen device
					(new org.openqa.selenium.interactions.touch.TouchActions(driver)).clickAndHold(sourceElement)
							.release(destinationElement).perform();
				}
			} catch (Exception e) {
	 ...
	 *
	 * @param targetElementLocator the locator of the webElement under test (By
	 ...
	 * Attempts to scroll the element into view in case of native mobile elements.
	 *
	 * @param targetElementLocator            the locator of the webElement under
	 *                                        test (By xpath, id, selector, name
	 *                                        ...etc)
	 * @param swipeDirection                  SwipeDirection.DOWN, UP, RIGHT, or
	 *                                        LEFT
	 * @param swipeTechnique                  SwipeTechnique.TOUCH_ACTIONS, or
	 *                                        UI_SELECTOR
	 * @param scrollableElementInstanceNumber in case of multiple scrollable views,
	 *                                        insert the instance number here
	 *                                        (starts with 0)
	 * @return a self-reference to be used to chain actions
	 */
	public TouchActions swipeElementIntoView(By targetElementLocator, SwipeDirection swipeDirection,
			SwipeTechnique swipeTechnique, int scrollableElementInstanceNumber) {
		By internalElementLocator = targetElementLocator;
		internalElementLocator = WebDriverElementActions.updateLocatorWithAIGeneratedOne(internalElementLocator);
		try {
			if (driver instanceof AppiumDriver<?>) {
				// appium native application
				boolean isElementFound = attemptToSwipeElementIntoViewInNativeApp(internalElementLocator,
						swipeDirection, swipeTechnique, scrollableElementInstanceNumber);
				...
				if (WebDriverElementActions.identifyUniqueElement(driver, internalElementLocator)) {
					...
					(new org.openqa.selenium.interactions.touch.TouchActions(driver))
							.scroll(elementLocation.getX(), elementLocation.getY()).perform();
				} else {
					WebDriverElementActions.failAction(driver, internalElementLocator);
				}
			}
	...
	private boolean attemptToSwipeElementIntoViewInNativeApp(By elementLocator, SwipeDirection swipeDirection,
			SwipeTechnique swipeTechnique, int scrollableElementInstanceNumber) {
		...
		do {
			...
			if (!driver.findElements(elementLocator).isEmpty()
				...
				&& attemptsToFindElement < DEFAULT_NUMBER_OF_ATTEMPTS_TO_SCROLL_TO_ELEMENT
				&& !lastPageSourceBeforeSwiping.equals(driver.getPageSource()));
	...
	private void attemptPinchToZoomOut() {
		...
		Sequence pinchAndZoom1 = new Sequence(finger, 0);
		pinchAndZoom1
				.addAction(finger.createPointerMove(Duration.ofMillis(0), PointerInput.Origin.viewport(), source.x / 3,
						source.y / 3))
				.addAction(finger.createPointerDown(PointerInput.MouseButton.LEFT.asArg()))
				.addAction(new Pause(finger, Duration.ofMillis(110)))
				.addAction(finger.createPointerMove(Duration.ofMillis(600), PointerInput.Origin.viewport(),
	...
	public enum KeyboardKeys {
		GO(ImmutableMap.of("action", "go")), DONE(ImmutableMap.of("action", "done")),
		SEARCH(ImmutableMap.of("action", "search")), SEND(ImmutableMap.of("action", "send")),
		NEXT(ImmutableMap.of("action", "next")), PREVIOUS(ImmutableMap.of("action", "previous")),
		NORMAL(ImmutableMap.of("action", "normal")), UNSPECIFIED(ImmutableMap.of("action", "unspecified")),
		NONE(ImmutableMap.of("action", "none"));
