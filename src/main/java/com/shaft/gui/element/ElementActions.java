...
public class ElementActions {
	...
	static void passAction(Screen screen, App applicationWindow, Pattern element, String testData) {
		String actionName = Thread.currentThread().getStackTrace()[2].getMethodName();
		passAction(null, null, actionName, testData,
				SikuliActions.prepareElementScreenshotAttachment(screen, applicationWindow, element, actionName, true));
	}
	...
	static void failAction(Screen screen, App applicationWindow, Pattern element, String testData,
			Exception... rootCauseException) {
		String actionName = Thread.currentThread().getStackTrace()[2].getMethodName();
		failAction(null, actionName, testData, null,
				SikuliActions.prepareElementScreenshotAttachment(screen, applicationWindow, element, actionName, false),
				rootCauseException);
	}

	private static void failAction(WebDriver driver, String actionName, String testData, By elementLocator,
			List<Object> screenshot, Exception... rootCauseException) {
	...
	private static boolean identifyUniqueElement(WebDriver driver, By elementLocator, int numberOfAttempts,
		...
		if (elementLocator != null) {
			...
			case 0:
				failAction(driver, "zero elements found matching this locator \"" + elementLocator + "\".",
						elementLocator);
				break;
			case 1:
		...
		try {
			new FluentWait<WebDriver>(driver)
					.withTimeout(Duration
							.ofSeconds((long) DEFAULT_ELEMENT_IDENTIFICATION_TIMEOUT_INTEGER * numberOfAttempts))
	...
	private static String confirmTypingWasSuccessful(WebDriver driver, By elementLocator, String expectedText,
			TextDetectionStrategy successfulTextLocationStrategy) {
		if (successfulTextLocationStrategy.equals(TextDetectionStrategy.UNDEFINED)) {
			successfulTextLocationStrategy = determineSuccessfulTextLocationStrategy(driver, elementLocator);
		}
	...
	private static void performHover(WebDriver driver, By elementLocator) {
		try {
			...
			javaScript = createMouseEvent + mouseEventFirstHalf + "mouseenter" + mouseEventSecondHalf
					+ dispatchMouseEvent;
			((JavascriptExecutor) driver).executeScript(javaScript, driver.findElement(elementLocator));

			javaScript = createMouseEvent + mouseEventFirstHalf + "mouseover" + mouseEventSecondHalf
					+ dispatchMouseEvent;
			((JavascriptExecutor) driver).executeScript(javaScript, driver.findElement(elementLocator));
	...
	public static void select(WebDriver driver, By elementLocator, String text) {
		if (identifyUniqueElement(driver, elementLocator)) {
			...
			try {
				...
				ReportManager.log(rootCauseException);
				failAction(driver, "waited for (" + DEFAULT_ELEMENT_IDENTIFICATION_TIMEOUT.getSeconds() + ") seconds",
	...
	public static String getAttribute(WebDriver driver, By elementLocator, String attributeName) {
		ReportManager.logDiscrete(
				"Attempting to getAttribute [" + attributeName + "] from elementLocator [" + elementLocator + "].");
		if (identifyUniqueElement(driver, elementLocator)) {
			// Override current locator with the aiGeneratedElementLocator
			elementLocator = updateLocatorWithAIGenratedOne(elementLocator);
			try {
	...
	public static String getSelectedText(WebDriver driver, By elementLocator) {
		if (identifyUniqueElement(driver, elementLocator)) {
			...
			try {
				new Select(driver.findElement(elementLocator)).getAllSelectedOptions()
						.forEach(selectedOption -> elementSelectedText.append(selectedOption.getText()));
	...
	public static void waitForElementToBePresent(WebDriver driver, By elementLocator, int numberOfTries,
		...
		ReportManager.logDiscrete("Waiting for element to be present; elementLocator [" + elementLocator
				+ "], numberOfTries[" + numberOfTries + "], stateOfPresence[" + stateOfPresence + "]...");
		// Override current locator with the aiGeneratedElementLocator
		elementLocator = updateLocatorWithAIGenratedOne(elementLocator);
		...
		String reportMessage = "waited up to (" + DEFAULT_ELEMENT_IDENTIFICATION_TIMEOUT.getSeconds() * numberOfTries
				+ ") seconds, for the element's state of presence to be (" + stateOfPresence + "). Element locator ("
	...
	public static boolean isElementClickable(WebDriver driver, By elementLocator) {
		if (identifyUniqueElement(driver, elementLocator) && driver.findElement(elementLocator).isEnabled()) {
			// Override current locator with the aiGeneratedElementLocator
			elementLocator = updateLocatorWithAIGenratedOne(elementLocator);
	 ...
	 * @return a TouchActions object capable of performing actions on touch-enabled
	 *         devices
	 */
