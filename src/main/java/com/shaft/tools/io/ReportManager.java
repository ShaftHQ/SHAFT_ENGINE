...
public class ReportManager {
			...
			attachments.forEach(attachment -> {
				if (attachment != null && attachment.get(2).getClass().toString().toLowerCase().contains("string")
					...
					if (attachment.get(2) instanceof byte[]) {
						attach(attachment.get(0).toString(), attachment.get(1).toString(),
								new ByteArrayInputStream((byte[]) attachment.get(2)));
					} else {
						attach(attachment.get(0).toString(), attachment.get(1).toString(),
	...
	private static void cleanAllureResultsDirectory() {
		...
		if (Boolean.TRUE.equals(Boolean.valueOf(System.getProperty("cleanAllureResultsDirectoryBeforeExecution")))) {
	...
	private static void downloadAndExtractAllureBinaries() {
		...
		allureBinaryPath = allureExtractionLocation + "allure-" + allureVersion + File.separator + "bin"
	...
	public static void prepareAllureReportingEnvironment() {
		...
		if (System.getProperty("executionAddress").trim().equals("local")
				|| (System.getProperty("mobile_platformName") != null
						&& !System.getProperty("mobile_platformName").trim().equals(""))) {
	 ...
	 * execution report.
	 *
	 * @param currentMethodName name of the current test method to be used in the
	 *                          attachment name
	 * @param testLog           content of the text log to be used as the attachment
	 *                          value
	 */
	...
	private static void createAllureReportArchiveAndCleanGeneratedDirectory() {
		if (FileActions.doesFileExist(allureExtractionLocation)) {
			...
			FileActions.zipFiles("generatedReport/",
					"generatedReport_" + new SimpleDateFormat("yyyyMMdd-HHmmss").format(new Date()) + ".zip");
		}
