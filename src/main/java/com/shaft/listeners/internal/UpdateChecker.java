...
public class UpdateChecker {
    ...
    public static void check() {
        ...
        Thread thread1 = Thread.ofVirtual().start(() -> {
            todayDate.set(new SimpleDateFormat("dd-MM-yyyy").format(new Date()));
        ...
        if (!doesEngineCheckFileExists.get() || !doesFileContainTodayDate.get()) {
            ...
            Thread.ofVirtual().start(() -> FileActions.getInstance(true).writeToFile("target/", lastCheckedFile, todayDate.get()));
