...
`shaft-intellij` is the stable IntelliJ IDEA plugin (`io.github.shafthq.shaft`, `10.3.20260703`). It is intentionally thin: first-run setup accepts a local SHAFT MCP stdio command generated outside IntelliJ, persists it, defaults to Codex CLI, and walks through `Choose assistant`, `Install MCP`, `Detect command`, and `Test connection` before revealing `Start chatting with SHAFT Assistant`. The plugin does not download or execute installer scripts at runtime. Settings remain available later for Local/Cloud routing, API keys, and custom local MCP commands.
