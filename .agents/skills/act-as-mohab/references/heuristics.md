...
Run the `verification-gap-lens.md` method for what to look for;
...
- **Orchestration** — dispatch through the role adapters
  (`coder`/`reviewer`/`tester`); each loads act-as-mohab itself. The
...
`scripts/agents/guard.py` R5 graphify / R6 TDD). Treat a hook reminder as a real
