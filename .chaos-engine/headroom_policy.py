...
def doctor_status() -> dict[str, object]:
    ...
    wrap = wrap_status()
    if status == "healthy" and wrap["status"] != "healthy":
        status = "recovery-required"
        detail = f"{detail}. {wrap['detail']}"
    return {
        "status": status,
        "taskImpact": "optional",
        "owner": "installer",
...
WRAP_HOSTS = ("claude", "codex", "grok", "copilot")


def wrap_command(host: str) -> str:
    return f'HEADROOM_SAVINGS_PROFILE=agent-90 HEADROOM_BEACON=off headroom wrap {host}'


def wrap_status() -> dict[str, object]:
    """Doctor: wrap/proxy must be how the host is launched (not raw CLI)."""
    marker = str(os.environ.get("HEADROOM_WRAP") or os.environ.get("HEADROOM_MODE") or "").strip()
    if marker:
        return {
            "status": "healthy",
            "detail": "Headroom wrap/proxy env present.",
            "command": wrap_command("grok"),
        }
    ...
    if which is None:
        return {
            "status": "absent",
            "detail": (
                "Headroom wrap not detected. Fix-next: "
                f"`{install_command()}` then `{wrap_command('grok')}` "
                "(or wrap claude|codex|copilot; Gemini: `headroom mcp serve`)."
            ),
            "command": wrap_command("grok"),
        }
    return {
        "status": "recovery-required",
        "detail": (
            "headroom CLI present but this process is not wrapped. "
            f"Launch via `{wrap_command('grok')}` (or matching host). "
            "Gemini: `headroom mcp serve`."
        ),
        "command": wrap_command("grok"),
    }
