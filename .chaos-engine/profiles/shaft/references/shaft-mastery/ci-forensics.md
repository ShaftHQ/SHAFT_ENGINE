...
shaft-pilot-release. E2E suites run on their existing nightly schedules; the
guidance validator is manual. So "CI is green" on a PR proves much less than
it suggests — run `scripts/ci/local_gate.py` (changed-module verify) and the
...
## Nightly-only E2E execution
Agents never manually start, rerun, or replace `E2E Tests` or `Local E2E Tests`,
including targeted dispatches. Prove a fix with the smallest directly affected
local headless test or local composite-action replay, then use exact-head PR
checks as the delivery gate. Existing scheduled nightly workflows are the sole
E2E execution owner. Agents may inspect scheduled results and fix failures;
they do not wait for, trigger, or substitute remote E2E proof.

## Local guard replay — prove a composite action's shell logic without dispatch (#4119)
This proves a *workflow-level guard* locally without running any remote job.
A composite action's `run:` step is plain shell once GitHub's
`${{ inputs.* }}` templating is peeled off — extract, parameterize, fixture,
replay.
...
5. Run the extracted script once per candidate input value and read the exit
   code and printed summary line. No runner queue or waiting.
...
filesystem state the shell actually branches on, and replay. Scheduled
nightly owns any later remote E2E evidence.
