...
| GAP-SESSIONSTART-STDOUT | Grok | high | Grok runs SessionStart but **ignores stdout**; `UserPromptSubmit` additionalContext is also discarded. CE injects the same locator card via always-on AGENTS.md and PreToolUse `hookSpecificOutput.additionalContext`. `HostCapability.session_start_stdout_honored=False`. Locator budget remains `SESSION_START_MAX_BYTES`=4096 on hosts that honor stdout. | `tests/scripts/test_chaos_engine_sessionstart_locator_parity.py`; Grok user-guide 10-hooks.md; kernel pin. |
| GAP-GROK-CAVEMAN | Grok | medium | Grok host communication constitution can override Caveman ultra in chat. Card + Headroom crush still apply. | Eval after wrap; do not duplicate Caveman body into AGENTS.md. |
