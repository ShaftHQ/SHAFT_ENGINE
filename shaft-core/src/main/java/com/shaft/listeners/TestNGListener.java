...
public class TestNGListener implements IAlterSuiteListener, IAnnotationTransformer,
    ...
    public static void engineSetup(ProjectStructureManager.RunType runType) {
        ...
        Properties.reporting.set().disableLogging(true);
        switch (runType) {
            case TESTNG ->
        ...
        Properties.reporting.set().disableLogging(false);
        ReportManagerHelper.logEngineVersion();
        Thread.ofVirtual().start(UpdateChecker::check);
        Thread.ofVirtual().start(() -> {
            try {
                var cls = Class.forName("com.shaft.gui.internal.image.ImageProcessingActions",
                        false, Thread.currentThread().getContextClassLoader());
                cls.getMethod("loadOpenCV").invoke(null);
        ...
        ReportManagerHelper.setDiscreteLogging(Properties.reporting.alwaysLogDiscreetly());
        ReportManagerHelper.setDebugMode(Properties.reporting.debugMode());
    }
    ...
    public void beforeInvocation(IInvokedMethod method, ITestResult iTestResult, ITestContext iTestContext) {
        ...
        if (Properties.reporting.debugMode()) {
            ReportManager.logDiscrete("elapsedTime: " + elapsedTime + "ms");
        }
        if (elapsedTime >= Properties.testNG.testSuiteTimeout() * 60000) {
            throw new SkipException("Skipping method as the test suite has exceeded the defined timeout of " + Properties.testNG.testSuiteTimeout() + " minutes.");
        }
...
//            TestNGListenerHelper.updateConfigurationMethodLogs(iTestResult);
    ...
    public void onExecutionFinish() {
        ...
        Thread.ofVirtual().start(() -> {
            try {
                var rbCls = Class.forName("com.shaft.api.RequestBuilder",
                        false, Thread.currentThread().getContextClassLoader());
                @SuppressWarnings("unchecked")
                Map<String, List<Double>> performanceData =
                        (Map<String, List<Double>>) rbCls.getMethod("getPerformanceData").invoke(null);
