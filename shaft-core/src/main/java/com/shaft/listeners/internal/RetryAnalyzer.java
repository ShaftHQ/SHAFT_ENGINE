 ...
 * {@code Properties.flags.set().retryMaximumNumberOfAttempts(n)} or the
...
public class RetryAnalyzer implements IRetryAnalyzer {
    ...
    public boolean retry(ITestResult iTestResult) {
        try {
            int maxRetryCount = Properties.flags.retryMaximumNumberOfAttempts();
    ...
    private void enableSupportingEvidenceCapture() {
        try {
            if (Properties.flags.forceCaptureSupportingEvidenceOnRetry()) {
                ...
                Properties.visuals.set().createAnimatedGif(true);
                Properties.visuals.set().screenshotParamsWhenToTakeAScreenshot("Always");
                Properties.visuals.set().whenToTakePageSourceSnapshot("FailuresOnly");
                Properties.reporting.set().captureWebDriverLogs(true);
            }
        } catch (Exception e) {
