...
public class JavaHelper {
    ...
    public static String formatLocatorToString(Object locator) {
        if (locator == null) return "";
        try {
            Class<?> relativeByClass = Class.forName(
                    "org.openqa.selenium.support.locators.RelativeLocator$RelativeBy",
                    false, Thread.currentThread().getContextClassLoader());
            if (relativeByClass.isInstance(locator)) {
                Object remoteParams = relativeByClass.getMethod("getRemoteParameters").invoke(locator);
                Object value = remoteParams.getClass().getMethod("value").invoke(remoteParams);
                return "Relative Locator: " + value;
            }
