 ...
 *             the 3.x binary exactly matches {@code Properties.internal.allure3Version()};
...
public class AllureManager {
    ...
    public static void initializeAllureReportingEnvironment() {
        ...
        allureResultsFolderPath = Properties.paths.allureResults();
     ...
     * {@code Properties.allure.automaticallyOpen()} is {@code true}.
    ...
    private static String renameAllureReport() {
        ...
        if (Properties.allure.accumulateReports())
     ...
     * {@code Properties.allure.generateArchive()} is {@code true}.
    ...
    public static void generateAllureReportArchive() {
        if (Properties.allure.generateArchive()) {
    ...
    private static void writeGenerateReportShellFilesToProjectDirectory() {
        ...
        String allure3Version = Properties.internal.allure3Version();
        boolean enforceConfiguredCliVersion = Properties.allure.forceConfiguredCliVersion();
    ...
    private static void cleanAllureResultsDirectory() {
        if (Properties.allure.cleanResultsDirectory()) {
    ...
    private static void writeAllureReport() {
        ...
        var customReportName = Properties.allure.customTitle();
    ...
    private static void startRealtimeMonitoringIfEligible() {
        if (!Properties.allure.realtimeMonitoring()) {
            return;
        }
        ...
        String command = prefix + " watch "
                + (Properties.allure.automaticallyOpen() ? "--open " : "")
                + "\"" + getResultsPath() + "\"";
    ...
    private static String resolveAllureCommandPrefix() {
        ...
        String allure3Version = Properties.internal.allure3Version();
        String nodeLtsVersion = Properties.internal.nodeLtsVersion();
    ...
    private static String getNodeJsDownloadUrl() {
        ...
        String version = Properties.internal.nodeLtsVersion();
        if (SystemUtils.IS_OS_WINDOWS) {
    ...
    private static String getNodeJsFolderName() {
        ...
        if (SystemUtils.IS_OS_WINDOWS) {
            ...
            return "node-v" + Properties.internal.nodeLtsVersion() + "-" + (isArm ? "win-arm64" : "win-x64");
        } else if (SystemUtils.IS_OS_MAC) {
            return "node-v" + Properties.internal.nodeLtsVersion() + "-" + (isArm ? "darwin-arm64" : "darwin-x64");
        } else {
            return "node-v" + Properties.internal.nodeLtsVersion() + "-" + (isArm ? "linux-arm64" : "linux-x64");
        }
    }
    ...
    private static void writeEnvironmentVariablesToAllureResultsDirectory() {
        ...
        propertiesFileBuilder.append("</environment>");
        String envXml = propertiesFileBuilder.toString();
        try {
            var cls = Class.forName("com.shaft.api.RestActions", false, Thread.currentThread().getContextClassLoader());
            envXml = (String) cls.getMethod("formatXML", String.class).invoke(null, envXml);
        } catch (ClassNotFoundException ignored) {
        } catch (Exception ignored) {
        }
        internalFileSession.writeToFile(Properties.paths.allureResults(), "environment.xml", envXml);
    }
}
