...
public class RealtimeReporter {
    ...
    public static void onExecutionFinished() {
        ...
        try {
            allureAutoOpen = Properties.allure.automaticallyOpen();
    ...
    public static boolean shouldLaunch() {
        try {
            if (!Properties.reporting.realtimeReport()) return false;
            if (EXECUTION_ADDRESS_LOCAL.equalsIgnoreCase(Properties.platform.executionAddress())
                    && Properties.web.headlessExecution()) {
                return false;
            }
        } catch (Exception e) {
            return false;
