...
- **Run Side-by-Side Comparison (before/after focused change)**: ``BEFORE=<before_commit> AFTER=<after_commit> TESTS='testPackage.ClassA,testPackage.ClassB' ; for C in $BEFORE $AFTER; do git checkout -q $C && START=$(date +%s) && mvn test -Dtest="$TESTS" -Dgpg.skip > /tmp/compare_$C.log 2>&1 ; END=$(date +%s) ; echo "$C,$((END-START))s" ; grep -E "Tests run:|BUILD SUCCESS|BUILD FAILURE" /tmp/compare_$C.log | tail -10 ; done``
