...
| `publish-intellij-plugin.yml` | Publish IntelliJ Plugin | Successful `Maven Central Continuous Delivery` run on `main`, plus manual | Builds, verifies, signs, and publishes the IntelliJ IDEA plugin to the JetBrains Marketplace Stable channel using repository secrets. | Downstream of Maven Central release. Keep it while JetBrains Marketplace remains the public IDE plugin distribution path. |
