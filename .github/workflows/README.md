...
| `agent-plugin-acceptance.yml` | Agent Plugin Live Acceptance | Weekly Monday 04:15 UTC, plus manual | Installs the pinned Claude Code and Codex CLI clients, reruns unauthenticated package/discovery/install evidence, and evaluates all 30 canonical routing cases when credentials exist before always uploading structured evidence. | Every native call shares a 900-second execution budget inside the 20-minute job: routing stops after 600 seconds, active setup stops with 120 seconds reserved for cleanup, and cleanup stops with 60 seconds for artifact writing. The job retains a further five-minute margin. Missing credentials, deadline exhaustion, and recognized provider/network outages are external blockers; an unrecognized nonzero client exit is a distinct gating client failure, never a fabricated routing miss. |
