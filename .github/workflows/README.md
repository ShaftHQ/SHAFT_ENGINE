...
| `live-tools-nightly.yml` | Live Tools Nightly E2E | Nightly, plus manual | Exercises the CLI and IntelliJ live-tool integrations and uploads every generated JaCoCo report on `main`. | Coverage failures remain non-fatal and are collected by `codecoverage-failure-notifier.yml`. |
| `guided-workflows-live.yml` | Guided Workflows Live E2E | Scheduled, plus manual | Runs the guided IntelliJ workflow acceptance suite and uploads its Gradle JaCoCo XML report on `main`. | Coverage failures remain non-fatal and are collected by `codecoverage-failure-notifier.yml`. |
| `codecoverage-failure-notifier.yml` | Code Coverage Failure Notifier | Completion of every workflow that uploads coverage | Downloads non-fatal JaCoCo/Codecov failure evidence and creates or comments on the one open `Code coverage reporting failures` issue. | Uses a fixed label and per-run marker for deduplication; it never closes the issue, so closure remains manual. |
...
| `agent-plugin-acceptance.yml` | Agent Plugin Live Acceptance | Weekly Monday 04:15 UTC, plus manual | Runs three independent evidence jobs: pinned native-client routing, the immutable external guardrail corpus, and checksum-verified agnix cross-client conformance. All upload structured evidence. | Native calls share a 900-second execution budget inside the 20-minute client job: routing stops after 600 seconds, active setup keeps 120 seconds for cleanup, and cleanup keeps 60 seconds for artifact writing. The guardrail job is a five-minute, token-free HTTPS read with no upstream runtime execution; all 15 excluded Git rows retain an explicit reason in its artifact. The agnix job stages copies outside canonical roots and runs its pinned binary in a no-network, read-only, nonroot container; exact known false positives are count-pinned and any new error fails. Missing credentials, deadline exhaustion, and recognized provider/network outages are external blockers; unrecognized client exits, corpus drift, and score-floor misses are gating failures. |
...
| `.github/actions/post-test-report` | `e2eTests.yml`, `e2eLocalTests.yml`, `lambdatestTests.yml` | Uploads JaCoCo coverage on `main`, creates Allure artifacts, writes summaries, and fails jobs from Allure/Surefire results. |
...
| `.github/actions/upload-jacoco-coverage` | Every JVM-test workflow, directly or through `post-test-report` | On `main`, discovers every Maven `jacoco.exec`, generates its XML report, includes Gradle XML reports, and uploads the complete job report set to Codecov. Generation/upload failures are recorded as artifacts without failing the test job. |
| `.github/actions/notify-codecoverage-failure` | `codecoverage-failure-notifier.yml` | Consolidates coverage failure artifacts into one manually closed tracking issue, with one comment per failed workflow run. |
