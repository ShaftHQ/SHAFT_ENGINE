...
| `agent-plugin-acceptance.yml` | Agent Plugin Live Acceptance | Weekly Monday 04:15 UTC, plus manual | Installs the pinned Claude Code and Codex CLI clients, reruns unauthenticated package/discovery/install evidence, attempts real skill loads when credentials exist, and always uploads structured evidence. | Missing model credentials are recorded as external blockers rather than package failures; package and client-command failures remain gating. |
