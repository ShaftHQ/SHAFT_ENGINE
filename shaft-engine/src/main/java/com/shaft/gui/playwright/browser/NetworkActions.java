...
/** Discoverable Playwright browser-network actions. */
public final class NetworkActions implements NetworkActionsContract {
    ...
    public NetworkActions offline() {
        perform("offline", () -> browser.getNativeContext().setOffline(true));
        return this;
    }

    @Override
    public NetworkActions online() {
        perform("online", () -> browser.getNativeContext().setOffline(false));
        return this;
    }

    @Override
    public NetworkActions throttle(long latencyMs, long downloadKbps, long uploadKbps) {
        return unsupported("throttling");
    }

    @Override
    public NetworkActions block(String... urlPatterns) {
        return unsupported("resource blocking");
    }
    ...
    private NetworkActions unsupported(String operation) {
        perform(operation, () -> {
            throw new UnsupportedOperationException("Network " + operation
                    + " is not supported by SHAFT's Playwright facade.");
        });
        return this;
    }
}
