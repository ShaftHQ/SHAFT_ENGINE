(channel, sink) => {
  ...
  // Issue #4271: this is NOT the authority on whether an id is safe to generate a locator from.
  // It only feeds the recorder's cosmetic overlay score; codegen re-checks every id against
  // LocatorPolicy.looksAutoGenerated (shaft-capture/.../generate/LocatorPolicy.java), which knows
  // the framework auto-id shapes this regex misses (React :r1:, mat-input-3, ember1234, id5, ...).
  // Deliberately single-sourced there: do not fork the pattern list into this file.
  const dynamic = value =>
    /[0-9]{8,}/.test(value || "") ||
