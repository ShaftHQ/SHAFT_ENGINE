...
/** Transaction coordinator for one exact iOS Simulator or Windows desktop setup plan. */
final class DesktopMobileSetupService {
    ...
    DesktopMobileSetupService(ShaftCachePaths paths, SetupPlan expectedPlan,
                              DesktopMobileToolchainOperations operations, boolean offline) {
    ...
    SetupReceipt install(SetupPlan plan, SetupApproval approval) throws IOException {
        ...
        Path lockPath = paths.state().resolve("desktop-mobile.lock").toAbsolutePath().normalize();
        ...
        try {
            ...
            throw new IOException("Interrupted while waiting for the desktop-mobile setup lock.", interrupted);
        } finally {
    ...
    private void requireCompatible(SetupPlan plan) {
        ...
        if (!expectedPlan.equals(plan)) {
            throw new IllegalArgumentException("Plan does not match the desktop-mobile manifest shipped with this release.");
        }
    }
    ...
    private void writeReceipt(SetupReceipt receipt) throws IOException {
        ...
        Path temporary = Files.createTempFile(paths.receipts(), receiptStem(), ".tmp");
        try {
    ...
    private Path receiptPath() {
        return paths.receipts().resolve(receiptStem() + ".json");
    }

    private Path previousReceiptPath() {
        return paths.receipts().resolve(receiptStem() + ".previous.json");
    }
    ...
    private static void requireDesktopProfile(SetupProfile profile) {
        if (profile != SetupProfile.MOBILE_IOS && profile != SetupProfile.MOBILE_WINDOWS) {
            throw new IllegalArgumentException("Desktop-mobile setup requires an iOS or Windows profile.");
        }
    }
