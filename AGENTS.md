...
At session start fetch/prune, branch/worktree fresh `ChaosEngine/*` from `origin/main`; reuse all session -- sub-tasks are commits. Before PR sync default, resolve conflicts, rerun checks; commit, push, open one ready PR per session. Draft only if blocked/incomplete/requested.
