#!/usr/bin/env python3
"""Probe optional local Colibri. Stdlib only. Never installs or starts it."""
...
DEFAULT_URL = "http://127.0.0.1:8000/v1/models"
DEFAULT_OPENAI_BASE = "http://127.0.0.1:8000/v1"
DEFAULT_ANTHROPIC_BASE = "http://127.0.0.1:8000/v1/messages"
TIMEOUT_SECONDS = 2
...
def coli_present() -> bool:
    """Return whether `coli` is on PATH. Equivalent to `command -v coli`; never executes it."""
    return shutil.which("coli") is not None
...
def classify(coli_on_path: bool, answered: bool, payload_ok: bool) -> str:
    if payload_ok:
        return "READY"
    if answered or coli_on_path:
        return "UNHEALTHY"
    return "ABSENT"
...
def probe(url: str = DEFAULT_URL, *, timeout: float = TIMEOUT_SECONDS) -> str:
    if not loopback_models_url(url):
        raise ValueError("non-loopback Colibri URL ignored")
    answered, body = fetch_models(url, timeout=timeout)
    return classify(coli_present(), answered, answered and is_models_payload(body))
...
def cmd_probe(args: argparse.Namespace) -> int:
    if not loopback_models_url(args.url):
        print("non-loopback Colibri URL ignored", file=sys.stderr)
        return 2
...
def cmd_models(args: argparse.Namespace) -> int:
    ...
    state = classify(coli_present(), answered, answered and is_models_payload(body))
    if state != "READY":
        print(state)
        return 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Ambient COLIBRI_BASE_URL / OPENAI_BASE_URL are intentionally unread.
