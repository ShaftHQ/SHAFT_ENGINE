<!-- Folded from skills/freetoken/SKILL.md (#6200). -->
# FreeToken
...
**Peer to Colibri.** FreeToken is the default local coding MoE path. Colibri (`:8000`) hosts frontier multitier MoE — see [colibri skill](colibri.md) and guide (repo-only `chaos-engine/guides/colibri.md`). Keep both; choose FreeToken for agency loops, Colibri for frontier MoE when the operator already serves it.
...
Probe helper: [`chaos-engine/skills/freetoken/scripts/probe.py`](../../freetoken/scripts/probe.py).
...
- OmniRoute (cloud-quota peer, not a dependency): [omniroute skill](omniroute.md)
- Local OpenAI-compat peers (Ollama / LM Studio / llamacpp): [local-openai-compat](local-openai-compat.md)
