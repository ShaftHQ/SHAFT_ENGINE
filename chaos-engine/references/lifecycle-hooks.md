...
| `PostToolUseFailure` | Record the failure and inject a pending reflection checkpoint when one is owed. Claude and Claude-compatible Grok use this native event. Codex reports both outcomes through `PostToolUse`; the guard classifies `tool_response`. |
