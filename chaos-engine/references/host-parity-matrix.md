...
| GAP-IMPL-COMPANIONS | all | medium | Implementation must load Caveman+Ponytail at ultra via portable overlay; not optional and not SessionStart-only. Doctor when companions missing without `--without-caveman/--without-ponytail`. | SKILL.md implement path + doctor; #5806. |
