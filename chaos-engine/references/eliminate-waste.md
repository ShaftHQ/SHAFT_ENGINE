...
- Keep the original issue intent and details. Ignore only the part that is waste (Headroom-style proxy). Do not rewrite the rest into a new program.
- Prefer a smaller diff that removes a hop over a larger diff that adds a wrapper.

## Do not

- Do not install, pin, wrap, vendor, or doctor-require a Headroom-style proxy. See [no-proxy](no-proxy.md).
