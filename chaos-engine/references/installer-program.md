...
2. Markers exist but the interior is neither the current owned block nor a
   ...
   Recognized-legacy fingerprints auto-upgrade the owned span in place:
   instruction markers must contain `Before every task, follow the canonical`
...
Required third parties self-heal via each item's official install command (CE vendor publish for companions). See `INSTALL.md` inventory and `official_self_heal.py`. Related: epic #5803, companions slice #5810.
