...
| Codex | [Configuration reference](https://developers.openai.com/codex/config-reference/) is the public configuration authority. OpenAI Codex source at [`5eea8d0`](https://github.com/openai/codex/commit/5eea8d0dd3f6b38b0e457d266fd7c918eb189bb6) defines 12 accepted hook names and maps tool results through `PostToolUse`, not `PostToolUseFailure`. Installed Codex 0.150.1 was checked live. | Codex registers `PreCompact`, omits the unsupported failure event, and decodes JSON-string `PostToolUse.tool_response` metadata before classifying nonzero exits; exact trigger and blocking regressions bind the projection. |
