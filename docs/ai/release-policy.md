...
Before release, confirm expected signals for affected paths:
- Allure result generation and attachments.
- Log4j/report output without sensitive data.
- Realtime dashboard connectivity/rendering if changed.
- CI summaries and artifact availability.
- Error rates/timeouts for external integrations where a safe staging/test account exists.
- Maven Central/GitHub release workflow status after publication.

The release owner is Mohab Mohie (`@MohabMohie`). TBD - needs team confirmation: define the post-release observation window in GitHub issue [#2804](https://github.com/ShaftHQ/SHAFT_ENGINE/issues/2804).
...
1. Verify the GitHub release/tag matches the `pom.xml` version.
2. Verify Maven Central contains POM, JAR, sources, JavaDocs, signatures, and checksums.
3. Resolve the release in a clean sample consumer and execute a minimal test.
4. Verify user-guide/sample version synchronization workflows.
5. Check JavaDocs publication.
6. Schedule non-blocking smoke paths for high-risk affected modules when useful.
7. Review workflow logs for accidental secret exposure and close/rotate immediately if detected.

The release owner provides final approval. The only blocking downstream behavior check is the SHAFT User Guide browser smoke test; all other smoke/matrix checks are scheduled and non-blocking.
