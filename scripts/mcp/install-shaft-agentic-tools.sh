...
fail() {
  echo "install-shaft-agentic-tools: $1" >&2
  exit "${2:-1}"
}
...
# downloads install-shaft-agentic-tools.sh into a scratch/temp directory instead, so this check reliably
# tells a genuine in-repo invocation apart from that scratch directory -- otherwise a stale
# install_shaft_agentic_tools.py left behind by an earlier remote run in the same temp directory would be
...
resolve_python_script() {
  ...
  if [ -n "$script_dir" ] && is_shaft_engine_repo_checkout "$script_dir" \
      && [ -f "$script_dir/install_shaft_agentic_tools.py" ]; then
    printf '%s\n' "$script_dir/install_shaft_agentic_tools.py"
    return 0
  fi
  ref="${SHAFT_MCP_INSTALLER_REF:-main}"
  url="${SHAFT_MCP_INSTALLER_PYTHON_URL:-https://raw.githubusercontent.com/ShaftHQ/SHAFT_ENGINE/$ref/scripts/mcp/install_shaft_agentic_tools.py}"
  target="$root/scripts/install_shaft_agentic_tools.py"
  download "$url" "$target"
  printf '%s\n' "$target"
}
