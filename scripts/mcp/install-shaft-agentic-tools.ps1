...
function Install-ShaftMcp {
    ...
    function Fail([string] $Message, [int] $Code = 1) {
        [Console]::Error.WriteLine("install-shaft-agentic-tools: $Message")
        exit $Code
    }
...
"@
    ...
    function Test-ShaftEngineRepoCheckout([string] $ScriptDirectory) {
        ...
        # always downloads install-shaft-agentic-tools.ps1 into a scratch/temp directory instead, so this
        # check reliably tells a genuine in-repo invocation apart from that scratch directory --
        # otherwise a stale install_shaft_agentic_tools.py left behind by an earlier remote run in the same
    ...
    function Resolve-PythonInstallerScript([string] $Root) {
        if ((-not [string]::IsNullOrWhiteSpace($PSScriptRoot)) -and (Test-ShaftEngineRepoCheckout $PSScriptRoot)) {
            $local = Join-Path $PSScriptRoot "install_shaft_agentic_tools.py"
        ...
        $url = if ([string]::IsNullOrWhiteSpace($env:SHAFT_MCP_INSTALLER_PYTHON_URL)) {
            "https://raw.githubusercontent.com/ShaftHQ/SHAFT_ENGINE/$ref/scripts/mcp/install_shaft_agentic_tools.py"
        } else {
            $env:SHAFT_MCP_INSTALLER_PYTHON_URL
        }
        $target = Join-Path $Root "scripts\install_shaft_agentic_tools.py"
        Save-Url $url $target
        return $target
    }
