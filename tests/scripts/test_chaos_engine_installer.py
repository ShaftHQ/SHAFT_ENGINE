...
class ChaosEngineInstallerTest(unittest.TestCase):
    ...
    def test_default_distribution_installs_only_neutral_portable_payload(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("Follow .chaos-engine/skills/chaos-engine/SKILL.md", second.stdout)
            ...
            main_write = subprocess.run(  # nosec B603
                ...
                input=json.dumps({"hook_event_name": "PreToolUse", "tool_name": "Write", "cwd": str(project), "tool_input": {"file_path": "x.txt"}, "session_id": "installed-main"}),
                capture_output=True, text=True, env=environment, check=False,
            )
            self.assertEqual(0, main_write.returncode)
            self.assertIn("Follow .chaos-engine/skills/chaos-engine/SKILL.md", main_write.stdout)
            ...
            self.assertEqual(0, branch_write.returncode, branch_write.stderr + branch_write.stdout)
            self.assertIn("Follow .chaos-engine/skills/chaos-engine/SKILL.md", branch_write.stdout)
    ...
    def test_stage_loss_fails_before_publish(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def copy_then_remove(source, destination):
                ...
                if Path(source).name == "install.py":
