...
class OrchestratorModeContractTest(unittest.TestCase):
    ...
    def test_orchestrator_follow_through_is_inspect_and_adapt_not_waiting(self):
        ...
        self.assertIn("5 minutes for short or high-risk work", policy)
        self.assertIn("10 minutes for normal work", policy)
        self.assertIn("15 minutes for long stable jobs", policy)
        self.assertIn("without the owner asking", policy)
        self.assertIn("Do not inspect more often than the selected cadence", policy)
    ...
    def test_delegation_pins_status_table_serial_cap_and_learning_session_before_kill(self):
        ...
        workflows = (CORE / "references/execution-workflows.md").read_text(encoding="utf-8")
        compact_workflows = re.sub(r"\s+", " ", workflows)
        self.assertIn("cap writers at four", compact_workflows)
        self.assertIn("independent, file-disjoint implementation units", compact_workflows)
        self.assertIn("Reduce parallel work to one implementer", compact_workflows)
